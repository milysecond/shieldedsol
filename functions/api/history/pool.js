...
export async function onRequest({ request, env }) {
  ...
  if (!protocol || !asset) return Response.json({ error: 'Protocol and asset required' }, { status: 400 });
  ...
  try {
    const rows = await getPoolHistory(protocol, asset, startDate.toISOString(), now.toISOString());
