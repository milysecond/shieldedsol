...
export default function Dashboard() {
  ...
  const getShareText = useCallback(
      ...
      const tvl = data ? fmtUsd(data.totalTvl) : '--';
        ...
        const tags =
          kind === 'x'
            ? ['@shieldedsol', leadHandle ? `@${leadHandle}` : null]
                .filter(Boolean)
                .join(' ')
            : Array.from(
                new Set(
      ...
      <main className="hero-main">
        ...
        <div className="follow-row">
          <div className="share-row" aria-label="Share">
            <button
              type="button"
              className="share-chip icon-only"
              onClick={() => openShare('x')}
              title="Share on X"
            ...
            <button
              type="button"
              className={`share-chip icon-only${copied ? ' done' : ''}`}
              onClick={() => openShare('copy')}
              title={copied ? 'Copied' : 'Copy stats'}
              aria-label={copied ? 'Copied' : 'Copy stats'}
            >
              {copied ? (
          ...
          <a
            ...
            title="Follow @shieldedsol on X"
          >
            <svg width="14" height="14" viewBox="0 0 24 24" fill="currentColor" aria-hidden>
              <path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z" />
            </svg>
            <span>@shieldedsol</span>
          </a>
        </div>
